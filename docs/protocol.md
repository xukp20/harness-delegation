...
Compact errors retain `code`, `message`, and available `details.source` / `details.native_stop_reason` to distinguish native failure origins. Full error details remain available in detail mode.
...
Grok uses an isolated HOME/GROK_HOME partitioned by canonical native auth-home identity, a native-auth symlink, `GROK_DISABLE_AUTOUPDATER=1`, no leader, and no inherited custom tool profile. Switching auth Home creates a separate native session store; it cannot silently retain the previous account link. Unsupported executable configuration is rejected from canonical cwd through the verified enclosing Git worktree root, inclusive. Without a reliable Git boundary, the full ancestor walk remains as a conservative fallback. No auth-home path is exempt: project configuration symlinks, including dangling links, remain rejected. This limitation is intentional: arbitrary hooks/MCP/plugins could run before a read-only prompt.

Guarded markers are `.grok/config.toml`, `.grok/hooks`, `.grok/plugins`, `.grok/lsp.json`, `.mcp.json`, `.claude/settings.json`, `.claude/settings.local.json`, `.claude/plugins`, `.cursor/mcp.json`, `.cursor/hooks.json` and `.envrc`. The guard is conservative (some loaders inspect only cwd or root), not an OS sandbox or a promise against concurrent filesystem changes. Native user configuration uses the isolated launch HOME/GROK_HOME, not a special-case exemption for the operator's home path.
