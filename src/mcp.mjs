...
export const tools = [
  ...
  tool('task_start', 'Start a bounded external harness job. Reuse request_key when retrying. Worker requires authorized write_scope.', { harness: { enum: ['pi', 'grok'] }, cwd: string, task: string, role: { enum: ['reviewer', 'explorer', 'worker'] }, request_key: string, write_scope: { type: 'array', items: string }, no_touch_scope: { type: 'array', items: string }, acceptance: { type: 'array', items: string }, timeout_seconds: { type: 'number' }, harness_options: { type: 'object', properties: { model: string, provider: string, thinking: string, tools: { type: 'array', items: string } }, additionalProperties: false } }, ['harness', 'cwd', 'task', 'request_key']),
  ...
  tool('task_read', 'Read bounded events after a cursor.', { job_id: string, after: { type: 'integer' }, limit: { type: 'integer' } }, ['job_id']),
  tool('task_wait', 'Wait at most 45 seconds for terminal or new events. Timeout never cancels work.', { job_id: string, after: { type: 'integer' }, timeout_seconds: { type: 'number', maximum: 45 } }, ['job_id']),
...
export async function dispatch(name, args = {}) {
  ...
  if (name === 'task_wait') return jobs.wait(job_id, options);
