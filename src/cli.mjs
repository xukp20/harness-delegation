...
export function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (['help', 'json', 'result'].includes(key)) { options[key] = true; continue; }
...
export async function execute(argv) {
  ...
  if (command === 'wait') return jobs.wait(id, { after: options.after !== undefined ? Number(options.after) : undefined, timeout_seconds: options.timeout_seconds !== undefined ? Number(options.timeout_seconds) : undefined });
